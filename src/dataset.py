...
class TrainBaseDataset:
    def __init__(self, data_path):
        ...
        for cust_id, value in self.data.items():
            ...
            for event_idx, (_, custinfo_idx, _) in filter(
                lambda x: x[1][0] == "custinfo", tmp
            ):
    ...
    def collate_fn(self, batch):
        ...
        for batch_id, (data, alert_event_idx) in enumerate(batch):
            ...
            for event in data["event_index"][s:e]:
                ...
                events[event_type]["num"].append(
                    torch.from_numpy(data[event_type]["num"][idx])
                )
                events[event_type]["cat"].append(
                    torch.from_numpy(data[event_type]["cat"][idx])
                )
